...
// ========== DIRECT PANEL CONFIGURATION (No PHP, no Cloudflare) ==========
const PANELS = [
  {
    name: 'KONEK',
...
async function fetchPanelMessages(panel, limit = 50) {
  try {
    ...
    // Expected format: { data: [ { dt, num, cli, message }, ... ] }
    ...
    // Fallback: maybe it's an array of arrays (old format)
    ...
    console.error(`Panel ${panel.name} fetch error:`, err.message);
    return [];
  }
}
...
// Get live OTP messages from both panels (direct fetch, no Cloudflare)
...
// Delete all numbers of a country (DELETE)
...
// ========== WEB UI for Admin (to add numbers easily) ==========
...
app.get('/', (req, res) => {
  res.send(`
    ...
    <body>
      ...
      <ul>
        <li><code>POST /api/admin/numbers</code> – add numbers (JSON body: { "country": "US", "numbers": ["+12025550123"] })</li>
